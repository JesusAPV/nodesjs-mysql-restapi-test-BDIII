...
export const updateAsistencia = async (req, res) => {
    try {
        const { id_asistencia_jp } = req.params;
        const { id_trabajador_jp, id_proyecto_jp, fecha_jp, horas_trabajadas_jp, estado_jp } = req.body;

        const [result] = await pool.query(
            `UPDATE tdasistencia_jp SET 
        id_trabajador_jp = COALESCE(?, id_trabajador_jp),
        id_proyecto_jp = COALESCE(?, id_proyecto_jp),
        fecha_jp = COALESCE(?, fecha_jp),
        horas_trabajadas_jp = COALESCE(?, horas_trabajadas_jp),
        estado_jp = COALESCE(?, estado_jp)
      WHERE id_asistencia_jp = ?`,
            [id_trabajador_jp, id_proyecto_jp, fecha_jp, horas_trabajadas_jp, estado_jp, id_asistencia_jp]
        );

        if (result.affectedRows === 0)
            return res.status(404).json({ message: 'Asistencia no encontrada' });

        const [rows] = await pool.query('SELECT * FROM tdasistencia_jp WHERE id_asistencia_jp = ?', [id_asistencia_jp]);
        res.json(rows[0]);
    } catch (error) {
        return res.status(500).json({ message: error.message });
    }
