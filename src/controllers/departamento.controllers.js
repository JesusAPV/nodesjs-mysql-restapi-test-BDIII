...
export const updateDepartamento = async (req, res) => {
  try {
    ...
    const [result] = await pool.query(
      'UPDATE tddepartamento_jp SET nombre_jp = ?, ubicacion_jp = ?, presupuesto_jp = ?, fecha_creacion_jp = ?, id_responsable_jp = ? WHERE id_departamento_jp = ?',
      [nombre_jp, ubicacion_jp, presupuesto_jp, fecha_creacion_jp, id_responsable_jp, id]
    );

    if (result.affectedRows === 0) return res.status(404).json({ message: 'Departamento no encontrado' });

    res.json({ id_departamento_jp: id, nombre_jp, ubicacion_jp, presupuesto_jp, fecha_creacion_jp, id_responsable_jp });
  } catch (error) {
    return res.status(500).json({ message: error.message });
  }
};
