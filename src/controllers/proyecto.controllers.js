...
export const updateProyecto = async (req, res) => {
    try {
        const { id_proyecto_jp } = req.params;
        const { nombre_jp, descripcion_jp, fecha_inicio_jp, fecha_fin_jp, id_departamento_jp } = req.body;

        const [result] = await pool.query(
            `UPDATE tdproyecto_jp SET 
        nombre_jp = COALESCE(?, nombre_jp), 
        descripcion_jp = COALESCE(?, descripcion_jp), 
        fecha_inicio_jp = COALESCE(?, fecha_inicio_jp), 
        fecha_fin_jp = COALESCE(?, fecha_fin_jp), 
        id_departamento_jp = COALESCE(?, id_departamento_jp)
      WHERE id_proyecto_jp = ?`,
            [nombre_jp, descripcion_jp, fecha_inicio_jp, fecha_fin_jp, id_departamento_jp, id_proyecto_jp]
        );

        if (result.affectedRows === 0)
            return res.status(404).json({ message: 'Proyecto no encontrado' });

        const [rows] = await pool.query('SELECT * FROM tdproyecto_jp WHERE id_proyecto_jp = ?', [id_proyecto_jp]);
        res.json(rows[0]);
    } catch (error) {
        return res.status(500).json({ message: error.message });
    }
